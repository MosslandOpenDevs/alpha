 ...
 *
 * ── Single-incident tool ────────────────────────────────────────────────
 * assertManifest()/assertCandidateAudit() pin this incident's exact counts
 * (5,400 pulses; 243 stored rows; 203 stale, 2 orphan, 22 relocated targets;
 * 263 rows and 225 AI runs afterwards). Every guard fails closed, so the
 * script simply refuses to run against any other drift set — a future repair
 * must re-derive those constants deliberately, not by editing one of them.
 *
 * ── Runbook ─────────────────────────────────────────────────────────────
 * Freeze alpha-why-moved-cron for the whole sequence (`pm2 stop
 * alpha-why-moved-cron`); a new article between the audit and the apply
 * changes the DB shape and every CAS check will refuse to proceed.
 *
 *   1. Backup production:
 *        sqlite3 "$DB_PATH" ".backup '<dir>/pre.sqlite'"
 *   2. Audit that backup — the manifest is bound to it by path + sha256:
 *        DB_PATH=<dir>/pre.sqlite pnpm tsx scripts/audit-why-moved-kst.ts \
 *          --out=<dir>/manifest.json
 *   3. Build the candidate DB (a copy of the backup with the corrected rows):
 *        cp <dir>/pre.sqlite <dir>/candidate.sqlite
 *        DB_PATH=<dir>/candidate.sqlite pnpm tsx scripts/generate-why-moved.ts --stale-only
 *        # relocated targets are not generated by any automatic mode:
 *        DB_PATH=<dir>/candidate.sqlite pnpm tsx scripts/generate-why-moved.ts <asset> <YYYY-MM-DD>
 *        # …once per manifest.relocatedMissingTargets entry, then delete the
 *        # orphaned UTC keys listed in the manifest:
 *        sqlite3 <dir>/candidate.sqlite \
 *          "DELETE FROM alpha_why_moved WHERE asset='<a>' AND date='<d>';"
 *   4. Re-audit the candidate (must report stale 0 / orphan 0):
 *        DB_PATH=<dir>/candidate.sqlite pnpm tsx scripts/audit-why-moved-kst.ts \
 *          --out=<dir>/candidate-audit.json
 *   5. Take the pre-merge backup immediately before applying (the SEO CAS
 *        snapshot): sqlite3 "$DB_PATH" ".backup '<dir>/premerge.sqlite'"
 *   6. Validate (read-only, no --apply), then apply:
 *        pnpm tsx scripts/apply-why-moved-kst-repair.ts \
 *          --manifest=<dir>/manifest.json --candidate-audit=<dir>/candidate-audit.json \
 *          --pre-backup=<dir>/pre.sqlite --premerge-backup=<dir>/premerge.sqlite \
 *          --candidate=<dir>/candidate.sqlite --receipt=<dir>/receipt.json [--apply]
 *   7. Post-check: re-run the audit against production (stale 0, orphan 0),
 *        `PRAGMA integrity_check`, then restart the cron.
 *
 * Keep <dir> (backups + receipt) for as long as rollback must stay possible:
 *   --rollback=<dir>/receipt.json --apply
 * A receipt with status "aborted" is refused — those writes never landed.
 */
...
type Receipt = {
  schemaVersion: 1;
  /**
   * prepared   — written inside the transaction; the process died before
   *              COMMIT, so production is at the before-state (crash path).
   * committed  — the repair is live.
   * rolled_back— a committed repair was reverted.
   * aborted    — a handled failure rolled the transaction back. Production
   *              is at the before-state and this receipt describes writes
   *              that never happened, so rollback must refuse it.
   */
  status: "prepared" | "committed" | "rolled_back" | "aborted";
  preparedAt: string;
  committedAt?: string;
  rolledBackAt?: string;
  abortedAt?: string;
...
function loadInputs() {
  ...
  // writeJsonExclusive() creates this directory later; create it up front so
  // realpathSync() gives a clear failure only for genuinely unusable paths.
...
function validateAndApply(): void {
  ...
  try {
    ...
    if (receiptWritten) {
      // The receipt describes an after-state that was never committed.
      // Mark it aborted so a later --rollback cannot replay its snapshots
      // against a different, successfully committed run.
      try {
        receipt!.status = "aborted";
        receipt!.abortedAt = new Date().toISOString();
        receipt!.abortReason = (error as Error)?.message ?? String(error);
        receipt!.receiptPayloadSha256 = receiptPayloadHash(receipt!);
        writeJsonAtomic(input.receiptPath, receipt!);
      } catch (receiptError) {
        console.error(
          "Failed to mark receipt aborted; do NOT roll back with it:",
          receiptError
        );
      }
    }
  ...
  if (sha256File(input.candidatePath) !== candidateHashBefore) {
    // The repair is already committed and verified; a candidate file that
    // moved afterwards is a housekeeping problem, not a failed repair.
    console.error(
      "WARNING: candidate changed after COMMIT; the repair itself is committed and verified."
    );
  }
...
function rollback(): void {
  ...
  if (receipt.schemaVersion !== 1) {
    fail("Unsupported repair receipt schema version");
  }
  if (receipt.status === "aborted") {
    fail(
      `Receipt was aborted before commit (${receipt.abortReason ?? "no reason recorded"}); ` +
        "production never received these writes, so there is nothing to roll back"
    );
  }
  if (!["prepared", "committed", "rolled_back"].includes(receipt.status)) {
    fail("Unsupported repair receipt status");
  }
